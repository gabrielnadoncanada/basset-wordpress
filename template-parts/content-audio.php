...
$lexend_audio_url = function_exists('get_field') ? get_field('fromate_style') : NULL;
...
<?php if (is_single()) : ?>
    ...
    <?php } else { ?>
        ...
        <article id="post-<?php the_ID(); ?>" <?php post_class('blog-post-item blog-details-wrap format-audio'); ?>>

            <?php if (!empty($lexend_audio_url)) : ?>
                <div class="blog__details-thumb postbox__audio ratio ratio-16x9">
                    <?php echo wp_oembed_get($lexend_audio_url); ?>
                </div>
            <?php endif; ?>
...
<?php else : ?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('blog-post-item post type-post panel vstack gap-3 rounded-2 p-3 pb-4 bg-secondary dark:bg-gray-300 dark:bg-opacity-10 format-audio'); ?>>
