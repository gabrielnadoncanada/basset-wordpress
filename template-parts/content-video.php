...
$lexend_video_url = function_exists('get_field') ? get_field('fromate_style') : NULL;
...
<?php if (is_single()) : ?>
    ...
    <?php } else { ?>
        ...
        <article id="post-<?php the_ID(); ?>" <?php post_class('blog-post-item blog-details-wrap format-video'); ?>>

            <?php if (has_post_thumbnail()) : ?>
                <div class="blog__details-thumb position-relative featured-image rounded ratio ratio-3x2 rounded-2 uc-transition-toggle overflow-hidden">
                    <?php the_post_thumbnail('full', ['class' => 'img-responsive']); ?>
                    <?php if (!empty($lexend_video_url)) : ?>
                        <a href="<?php print esc_url($lexend_video_url); ?>" class="video--icon popup-video"><i class="fas fa-play"></i></a>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
...
<?php else : ?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('blog-post-item post type-post panel vstack gap-3 rounded-2 p-3 pb-4 bg-secondary dark:bg-gray-300 dark:bg-opacity-10 format-video'); ?>>
